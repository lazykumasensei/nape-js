...
export default {
  ...
  code2d: `// Slingshot Siege — launch balls to shatter a castle
const W = canvas.width, H = canvas.height;
const space = new Space(new Vec2(0, 600));
addWalls();

const SLING_X = 120, BALL_R = 7, MAX_PULL = 120, POWER = 18;
const slingY = H - 100;
const cbBall = new CbType(), cbCastle = new CbType(), cbWall = new CbType();

// Tag walls so castle pieces fracture on floor impact
for (const b of space.bodies) if (b.isStatic()) b.cbTypes.add(cbWall);

function addBox(x, y, w, h) {
  const b = new Body(BodyType.DYNAMIC, new Vec2(x, y));
  b.shapes.add(new Polygon(Polygon.box(w, h)));
  b.userData._breakable = true;
  b.cbTypes.add(cbCastle);
  b.space = space;
}

function doFracture(body, ix, iy, impulse) {
  if (!body.space || !body.userData._breakable) return;
  try {
    const r = fractureBody(body, Vec2.get(ix, iy),
      { fragmentCount: 4, explosionImpulse: impulse });
    r.fragments.forEach(f => {
      f.userData._breakable = f.shapes.at(0).area >= 250;
      if (f.userData._breakable) f.cbTypes.add(cbCastle);
    });
  } catch {}
}

// Castle — 3-level tower
const cx = W - 200, fy = H - 20;
const widths = [160, 120, 80];
let y = fy;
for (const w of widths) {
  addBox(cx - w/2 + 6, y - 25, 12, 50);
  addBox(cx + w/2 - 6, y - 25, 12, 50);
  addBox(cx, y - 58, w, 16);
  y -= 66;
}
addBox(cx, y - 14, 40, 24);

// Slingshot ball
let ball = new Body(BodyType.KINEMATIC, new Vec2(SLING_X, slingY));
ball.shapes.add(new Circle(BALL_R, undefined, new Material(0.3, 0.6, 0.4, 3)));
ball.isBullet = true;
ball.cbTypes.add(cbBall);
ball.userData._ball = true;
...
let pulling = false, pullX = SLING_X, pullY = slingY, launched = false;

// Ball hits castle
space.listeners.add(new InteractionListener(
  CbEvent.BEGIN, InteractionType.COLLISION, cbBall, cbCastle, (cb) => {
    const b1 = cb.int1.castBody ?? cb.int1.castShape?.body;
    const b2 = cb.int2.castBody ?? cb.int2.castShape?.body;
    if (!b1 || !b2) return;
    const castle = b1.userData._ball ? b2 : b1;
    const bball = b1.userData._ball ? b1 : b2;
    const rv = { x: bball.velocity.x - castle.velocity.x, y: bball.velocity.y - castle.velocity.y };
    if (Math.sqrt(rv.x*rv.x + rv.y*rv.y) < 80) return;
    const mx = (b1.position.x + b2.position.x) / 2;
    const my = (b1.position.y + b2.position.y) / 2;
    setTimeout(() => doFracture(castle, mx, my, 40), 0);
  },
));

// Castle-castle (chain reactions)
space.listeners.add(new InteractionListener(
  CbEvent.BEGIN, InteractionType.COLLISION, cbCastle, cbCastle, (cb) => {
    const b1 = cb.int1.castBody ?? cb.int1.castShape?.body;
    const b2 = cb.int2.castBody ?? cb.int2.castShape?.body;
    if (!b1 || !b2) return;
    const rv = { x: b1.velocity.x - b2.velocity.x, y: b1.velocity.y - b2.velocity.y };
    if (Math.sqrt(rv.x*rv.x + rv.y*rv.y) < 200) return;
    const mx = (b1.position.x + b2.position.x) / 2;
    const my = (b1.position.y + b2.position.y) / 2;
    const s1 = b1.velocity.x*b1.velocity.x + b1.velocity.y*b1.velocity.y;
    const s2 = b2.velocity.x*b2.velocity.x + b2.velocity.y*b2.velocity.y;
    setTimeout(() => doFracture(s1 > s2 ? b2 : b1, mx, my, 15), 0);
  },
));

// Castle hits wall/floor (hard landing)
space.listeners.add(new InteractionListener(
  CbEvent.BEGIN, InteractionType.COLLISION, cbCastle, cbWall, (cb) => {
    const b1 = cb.int1.castBody ?? cb.int1.castShape?.body;
    const b2 = cb.int2.castBody ?? cb.int2.castShape?.body;
    if (!b1 || !b2) return;
    const castle = b1.userData._breakable ? b1 : b2;
    if (!castle.userData._breakable) return;
    const v = castle.velocity;
    if (Math.sqrt(v.x*v.x + v.y*v.y) < 200) return;
    setTimeout(() => doFracture(castle, castle.position.x, castle.position.y, 10), 0);
  },
));
...
canvasWrap.addEventListener("pointerdown", (e) => {
  ...
  pullX = (e.clientX - rect.left) * sx;
  pullY = (e.clientY - rect.top) * sy;
  const dx = pullX - SLING_X, dy = pullY - slingY;
  if (launched && dx*dx+dy*dy < 3600) {
    ball = new Body(BodyType.KINEMATIC, new Vec2(SLING_X, slingY));
    ball.shapes.add(new Circle(BALL_R, undefined, new Material(0.3, 0.6, 0.4, 3)));
    ball.isBullet = true; ball.cbTypes.add(cbBall);
    ball.userData._ball = true; ball.space = space;
    launched = false;
  }
  if (!launched) pulling = true;
});
...
canvasWrap.addEventListener("pointermove", (e) => {
  ...
  const dx = mx - SLING_X, dy = my - slingY;
  const d = Math.sqrt(dx*dx+dy*dy);
  if (d > MAX_PULL) { mx = SLING_X + dx/d*MAX_PULL; my = slingY + dy/d*MAX_PULL; }
  pullX = mx; pullY = my;
  ball.position.x = pullX; ball.position.y = pullY;
});

canvasWrap.addEventListener("pointerup", () => {
  if (!pulling) return;
  pulling = false;
  const dx = SLING_X - pullX, dy = slingY - pullY;
  if (Math.sqrt(dx*dx+dy*dy) < 10) return;
  ball.type = BodyType.DYNAMIC;
  ball.velocity = new Vec2(dx * POWER, dy * POWER);
  launched = true;
});
...
function loop() {
  ...
  ctx.beginPath(); ctx.moveTo(SLING_X, slingY+20); ctx.lineTo(SLING_X-20, slingY-10); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(SLING_X, slingY+20); ctx.lineTo(SLING_X+20, slingY-10); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(SLING_X, slingY+20); ctx.lineTo(SLING_X, slingY+80); ctx.stroke();
  if (pulling) {
    ctx.strokeStyle = "#c44"; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.moveTo(SLING_X-20, slingY-10); ctx.lineTo(pullX, pullY); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(SLING_X+20, slingY-10); ctx.lineTo(pullX, pullY); ctx.stroke();
  }
...
loop();`,
  ...
  codePixi: `// Slingshot Siege — launch balls to shatter a castle
const space = new Space(new Vec2(0, 600));
addWalls();
