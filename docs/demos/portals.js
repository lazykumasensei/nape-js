...
export default {
  ...
  code2d: `// Portals — full portal physics with UserConstraint + callbacks
...
class PortalConstraint extends UserConstraint {
  ...
  _portalBody1 = null; _portalBody2 = null;
  _position1 = null; _position2 = null;
  _direction1 = null; _direction2 = null;
  _scale = 1;
  _unitDir1 = null; _unitDir2 = null;
  _p1 = null; _p2 = null;
  _s1 = null; _s2 = null;
  _n1 = null; _n2 = null;
  _a1 = 0; _a2 = 0;

  constructor(pb1, pos1, dir1, pb2, pos2, dir2, scale, b1, b2) {
    super(3);
    this.body1 = b1; this.body2 = b2;
    this.portalBody1 = pb1; this.portalBody2 = pb2;
    this.position1 = pos1; this.position2 = pos2;
    this.direction1 = dir1; this.direction2 = dir2;
    this._scale = scale;
    this._unitDir1 = new Vec2(); this._unitDir2 = new Vec2();
    this._p1 = new Vec2(); this._p2 = new Vec2();
    this._s1 = new Vec2(); this._s2 = new Vec2();
    this._n1 = new Vec2(); this._n2 = new Vec2();
  }
  ...
  setProperties(clone, original) {
    this.__validate(); this.__prepare();
    const v = [0, 0, 0]; this.__velocity(v);
    const { _p1: p1, _p2: p2, _n1: n1, _n2: n2, _s1: s1, _s2: s2, _a1: a1, _a2: a2, _scale: sc } = this;
    if (clone === this._body2) {
      const s1dn1 = s1.dot(n1), s1cn1 = s1.cross(n1);
      clone.position = new Vec2(
        this._portalBody2.position.x + p2.x - (n2.x * s1dn1 + n2.y * s1cn1) * sc,
        this._portalBody2.position.y + p2.y - (n2.y * s1dn1 - n2.x * s1cn1) * sc,
      );
      ...
      const s2dn2 = s2.dot(n2), s2cn2 = s2.cross(n2);
      clone.position = new Vec2(
        this._portalBody1.position.x + p1.x - (n1.x * s2dn2 + n1.y * s2cn2) / sc,
        this._portalBody1.position.y + p1.y - (n1.y * s2dn2 - n1.x * s2cn2) / sc,
      );
  ...
  __validate() {
    const len1 = this._direction1.length, len2 = this._direction2.length;
    this._unitDir1.setxy(this._direction1.x / len1, this._direction1.y / len1);
    this._unitDir2.setxy(this._direction2.x / len2, this._direction2.y / len2);
  }
  ...
  __prepare() {
    let tmp;
    tmp = this._portalBody1.localVectorToWorld(this._position1); this._p1.set(tmp); tmp.dispose();
    tmp = this._portalBody2.localVectorToWorld(this._position2); this._p2.set(tmp); tmp.dispose();
    const b1pos = this._body1.position, b2pos = this._body2.position;
    const pb1pos = this._portalBody1.position, pb2pos = this._portalBody2.position;
    this._s1.setxy(b1pos.x - this._p1.x - pb1pos.x, b1pos.y - this._p1.y - pb1pos.y);
    this._s2.setxy(b2pos.x - this._p2.x - pb2pos.x, b2pos.y - this._p2.y - pb2pos.y);
    tmp = this._portalBody1.localVectorToWorld(this._unitDir1); this._n1.set(tmp); tmp.dispose();
    tmp = this._portalBody2.localVectorToWorld(this._unitDir2); this._n2.set(tmp); tmp.dispose();
    this._a1 = this._unitDir1.angle + this._portalBody1.rotation;
    this._a2 = this._unitDir2.angle + this._portalBody2.rotation;
  }

  __position(err) {
    const { _s1: s1, _s2: s2, _n1: n1, _n2: n2, _scale: sc } = this;
    err[0] = sc * s1.dot(n1) + s2.dot(n2);
    err[1] = sc * s1.cross(n1) + s2.cross(n2);
    err[2] = (this._body1.rotation - this._a1) - (this._body2.rotation - this._a2) - Math.PI;
  }
  ...
  __velocity(err) {
    const b1v = this._body1.constraintVelocity, b2v = this._body2.constraintVelocity;
    const b1w = this._body1.angularVel + this._body1.kinAngVel;
    const b2w = this._body2.angularVel + this._body2.kinAngVel;
    const pb1v = this._portalBody1.constraintVelocity, pb2v = this._portalBody2.constraintVelocity;
    const pb1w = this._portalBody1.angularVel + this._portalBody1.kinAngVel;
    const pb2w = this._portalBody2.angularVel + this._portalBody2.kinAngVel;
    const { _p1: p1, _p2: p2, _s1: s1, _s2: s2, _n1: n1, _n2: n2, _scale: sc } = this;
    const u1x = b1v.x - (-pb1w * p1.y) - pb1v.x;
    const u1y = b1v.y - (pb1w * p1.x) - pb1v.y;
    const u2x = b2v.x - (-pb2w * p2.y) - pb2v.x;
    const u2y = b2v.y - (pb2w * p2.x) - pb2v.y;
    err[0] = sc * (u1x * n1.x + u1y * n1.y + pb1w * s1.cross(n1)) + (u2x * n2.x + u2y * n2.y + pb2w * s2.cross(n2));
    err[1] = sc * (u1x * n1.y - u1y * n1.x + pb1w * s1.dot(n1)) + (u2x * n2.y - u2y * n2.x + pb2w * s2.dot(n2));
    err[2] = (b1w - pb1w) - (b2w - pb2w);
  }
...
class PortalManager {
  ...
  _portalBegin(cb) {
    ...
    if (portalPair == null && shapeData.__portal_id != null && shape.cbTypes.has(this.PARTIAL)) {
      for (const pData of shapeData.__portals) {
        if (this._behindPortal(pData.portal, portal.sensor.worldCOM)) return;
      }
    }
    ...
    if (portalPair == null) {
      if (bodyData.__portal_pairs != null) {
        ...
        while (stack.length > 0 && !longChain) {
          ...
          for (const pair of curData.__portal_pairs) {
            const otherBody = pair.bodyA === cur ? pair.bodyB : pair.bodyA;
            if (visited.indexOf(otherBody) === -1) { visited.push(otherBody); stack.push(otherBody); }
            if (pair.portalA === portal || pair.portalB === portal) { longChain = true; break; }
          }
        }
        if (longChain) return;
      }
      ...
      portalPair = { portalA: portal, portalB: targetPortal, bodyA: body, bodyB: clone };
      if (bodyData.__portal_pairs == null) bodyData.__portal_pairs = [];
      bodyData.__portal_pairs.push(portalPair);
      clone.userData.__portal_pairs = [portalPair];

      let id = shapeData.__portal_id, portals = shapeData.__portals;
      if (id == null) {
        id = shapeData.__portal_id = this._portalID++;
        portals = shapeData.__portals = [];
        shapeData.__portal_active = 0;
      ...
      if (shapeData.__portal_id != null) {
        for (const cloneShape of clone.shapes) {
          if (cloneShape.userData.__portal_id === shapeData.__portal_id) { found = true; break; }
        }
      }
  ...
  _portalEnd(cb) {
    ...
    while (stack.length > 0) {
      ...
      for (const pair of stackData.__portal_pairs) {
        const cloneBody = pair.bodyA === stackBody ? pair.bodyB : pair.bodyA;
        for (const cloneShape of cloneBody.shapes) {
          if (cloneShape.userData.__portal_active != null) {
            if (shapes.indexOf(cloneShape) === -1) { shapes.push(cloneShape); stack.push(cloneShape.body); }
          }
        }
      }
    }
    ...
    for (const s of shapes) {
      let anyTarget = false;
      for (const pData of s.userData.__portals) {
        if (pData.portal_target) { anyTarget = true; break; }
      }
      if (anyTarget) {
        const sBody = s.body;
    ...
    for (const survivor of survivors) {
      ...
      if (bData.__portal_pairs) {
        ...
        while (i < bData.__portal_pairs.length) {
          const pair = bData.__portal_pairs[i];
          const clone = pair.bodyA === survBody ? pair.bodyB : pair.bodyA;
          if (clone.space == null) bData.__portal_pairs.splice(i, 1);
          else i++;
        }
      }
    }
  ...
  _behindPortal(portal, position) {
    const wp = portal.body.localPointToWorld(portal.position);
    const ux = position.x - wp.x, uy = position.y - wp.y;
    const dir = portal.body.localVectorToWorld(portal.direction);
    const behind = ux * dir.x + uy * dir.y <= 0;
    wp.dispose(); dir.dispose();
    return behind;
  }
  ...
  _handlePartial(partial, carb, ret) {
    ...
    for (const portalData of portals) {
      ...
      while (i < carb.contacts.length) {
        ...
        const py = contact.position.y + carb.normal.y * contact.penetration * scale;
        if (this._behindPortal(portal, { x: px, y: py })) {
          carb.contacts.remove(contact); anyBehind = true;
        } else i++;
      }
      if (anyBehind) {
        let j = 0;
        while (j < carb.contacts.length) {
          const contact = carb.contacts.at(j);
          if (contact.penetration < 0) carb.contacts.remove(contact);
          else j++;
        }
      }
      if (carb.contacts.empty()) return PreFlag.IGNORE_ONCE;
  ...
  _backCollision(cb) {
    const carb = cb.arbiter.collisionArbiter;
    if (!carb) return PreFlag.ACCEPT_ONCE;
    const s1 = cb.arbiter.shape1, s2 = cb.arbiter.shape2;
    let partial, other;
    if (s1.userData.__portals && s1.userData.__portals.length > 0) { partial = s1; other = s2; }
    else if (s2.userData.__portals && s2.userData.__portals.length > 0) { partial = s2; other = s1; }
    else return PreFlag.ACCEPT_ONCE;

    for (const portalData of partial.userData.__portals) {
      const portal = portalData.portal;
      if (other.body === portal.body) {
        if (portal.collide.indexOf(other) !== -1) return PreFlag.ACCEPT_ONCE;
        if (portal.ignore.indexOf(other) !== -1) return PreFlag.IGNORE_ONCE;
...
// ── Portal generation ───────────────────────────────────────────────────
const manager = new PortalManager(space);
...
function genPortal(width, position, rotation, body) {
  ...
  const back = new Polygon(Polygon.rect(-10, -width / 2, 5, width));
  sides[0].rotate(rotation); sides[1].rotate(rotation); back.rotate(rotation);
  if (position != null) {
    sides[0].translate(position); sides[1].translate(position); back.translate(position);
  }
  sides[0].body = body; sides[1].body = body; back.body = body;
  ...
  const portal = {
    target: null, collide: sides, ignore: [back],
    body, sensor, position: pos, direction: dir, width,
  };
  sensor.userData.__portal = portal;
  return portal;
}

// Create portal body with 4 portals in X-layout
const portalBody = new Body(BodyType.STATIC);
const portalA  = genPortal(120, new Vec2(200, 100),     0,       portalBody);
const portalA2 = genPortal(120, new Vec2(W - 200, H - 120), Math.PI, portalBody);
const portalB  = genPortal(120, new Vec2(200, H - 120), 0,       portalBody);
...
// ── Spawn shapes ────────────────────────────────────────────────────────
function spawnShape(x, y, type) {
  const body = new Body();
  const r = 12;
  if (type === "circle") body.shapes.add(new Circle(r));
  else if (type === "box") body.shapes.add(new Polygon(Polygon.box(r * 2, r * 2)));
  else body.shapes.add(new Capsule(r * 2.5, r * 1.4));
  body.position = new Vec2(x, y);
  body.space = space;
  for (const s of body.shapes) s.cbTypes.add(manager.PORTABLE);
}

for (let i = 0; i < 5; i++) {
  spawnShape(W / 2 - 80 + i * 40, H / 2 - 60, "circle");
  spawnShape(W / 2 - 80 + i * 40, H / 2,      "box");
  spawnShape(W / 2 - 80 + i * 40, H / 2 + 60,  "capsule");
}

// ── Drag interaction ────────────────────────────────────────────────────
const mouseBody = new Body(BodyType.KINEMATIC, new Vec2(-1000, -1000));
mouseBody.space = space;
let grabJoint = null, pendingGrab = null, pendingRelease = false;
let dragX = 0, dragY = 0;
...
canvasWrap.addEventListener("pointerdown", (e) => {
  ...
  if (best) pendingGrab = { body: best, localPt: best.worldPointToLocal(new Vec2(dragX, dragY)) };
});
canvasWrap.addEventListener("pointermove", (e) => {
  const r = canvas.getBoundingClientRect();
  dragX = e.clientX - r.left; dragY = e.clientY - r.top;
});
canvasWrap.addEventListener("pointerup", () => { pendingRelease = true; });

// ── Portal glow renderer ────────────────────────────────────────────────
...
function loop() {
  ...
  if (pendingRelease) {
    ...
    mouseBody.position.setxy(-1000, -1000);
    mouseBody.velocity.setxy(0, 0);
  }
  if (pendingGrab) {
    const { body, localPt } = pendingGrab;
    pendingGrab = null;
    if (grabJoint) { grabJoint.space = null; grabJoint = null; }
    mouseBody.position.setxy(dragX, dragY);
    const j = new PivotJoint(mouseBody, body, new Vec2(0, 0), localPt);
    j.stiff = false; j.frequency = 6; j.damping = 1;
    grabJoint = j; j.space = space;
  }
  if (grabJoint) {
    ...
    if (dist > 1) {
      const speed = Math.min(dist * 60, 1200);
      mouseBody.velocity.setxy(dx / dist * speed, dy / dist * speed);
