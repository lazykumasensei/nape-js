...
export default {
  ...
  code2d: `// Performance Profiler demo
const W = canvas.width, H = canvas.height;
const space = new Space(new Vec2(0, 600));

// Enable profiler — zero overhead when disabled
space.profilerEnabled = true;
...
function loop() {
  ...
  // Read profiler metrics after step()
  const m = space.metrics;
  ctx.fillStyle = "rgba(0,0,0,0.8)";
  ctx.fillRect(10, 10, 250, 80);
  ctx.fillStyle = "#0f8";
  ctx.font = "11px monospace";
  ctx.fillText(\`Step: \${m.totalStepTime.toFixed(2)} ms\`, 20, 28);
  ctx.fillText(\`Bodies: \${m.bodyCount}  Contacts: \${m.contactCount}\`, 20, 44);
  ctx.fillText(\`Broad: \${m.broadphaseTime.toFixed(2)}  Narrow: \${m.narrowphaseTime.toFixed(2)}\`, 20, 60);
  ctx.fillText(\`VelSolve: \${m.velocitySolverTime.toFixed(2)}  PosSolve: \${m.positionSolverTime.toFixed(2)}\`, 20, 76);
